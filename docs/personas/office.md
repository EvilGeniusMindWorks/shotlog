...
- **Push 2:** the Jobsite Contact Sheet as a paper of every job — his rows exactly, prefilled from the company, the site and the customer with source chips, override on the job only, "Use the site's again", "Make this the site's too", a later site change offered per job, dated versions, the print with the route on the back, the crew's ☎ on the day. **Push 3:** Suggest on the Hospital and Urgent Care rows — the nearest three with an ER from the bundled federal hospital list, urgent care from OpenStreetMap (Search the web when a phone is not tagged); the back page prints a scan-for-directions code per place and, once the company's router is on, the drive one step per line.
