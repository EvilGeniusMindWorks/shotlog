...
- `69` — Round S11 — the first week's small fixes: A job needs a site (required field, no nameless customer, move job to another site); Weights to four decimals on screen, print and PDF; PF from exact pounds; First-week card leaves on done / first filed day / 14 days, back from ? menu; Nearby jobs in the Which job? sheet, save the site's spot; Crash reporting: a thrown error reaches the reporter (stub) on web and API.
