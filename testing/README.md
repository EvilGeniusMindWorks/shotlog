...
- `83` — Round S22 — The job page shows its facts, a job is set up in a minute, and every job carries its contact sheet: Job, customer and site pages: facts in the header, real Overview cards, wide forms; New job in one sheet: customer, site, job, then the contact sheet; Jobs rows expand in place; the setup line; The Jobsite Contact Sheet is a paper of the job: prefilled rows with source chips, override on the job only, Use the site's again; When a site row changes later, the jobs using it are asked; Make this the site's too; dated versions and the print; The crew's phone: the day's ☎ shows the sheet offline, one tap to call, a tap opens the device's maps; Admin › Company: the BBI Office rows and Direct Contractor prefill every sheet; Nearest hospital with an ER from the federal list, urgent care from OpenStreetMap; Suggest fills the row; The route on the back: our own router, one step per line in readable type, and the QR code.
