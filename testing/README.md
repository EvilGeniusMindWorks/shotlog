...
- `82` — Round S21 — Records that fill the window, attachments under the PDF, and the approval process: Records fills the window: two-line rows, the Columns menu and density; The tree navigator customer › site › job › day beside the columns; The drawer preview sticks, Open in a window, paper-clip counts; The filmstrip: attachments under the PDF with their context, kind chips, search and the lightbox; Export binder carries the index; Admin › Company › Approvals: the matrix, office approves everything; The review screen: papers left, PDF and attachments right, Approve and Send back with a note; The filer's home shows the note and Approved by; only an approver sends back; the queue's columns, Mine and Print pack.
