async (page, lib) => {
  ...
  await R.section("Rigs: today's rigs with each machine's start and stop readings, another rig, out of service; the drill log asks for no meter", async () => {
    ...
    await openDay(PB);
    R.ok('the rigs tile says open (S20: waiting for stop hours, nothing "running")', (await PB.locator('[data-tile="rigs"]').getAttribute('data-tile-state')) === 'open');
    const row1 = PB.locator(`[data-rig-row="${rigs.first.asset}"]`);
    R.ok(`${rigs.first.asset}'s row: walk-around · ${start1} → stop hours missing`, /stop hours missing/.test(await row1.innerText()) && (await row1.innerText()).includes(fmtH(start1)) && !/running/.test(await row1.innerText()));
    await row1.click();
    ...
    await openDay(PB);
    const stopped = await waitFor(() => row1.innerText().then((t) => (/filed/.test(t) ? t : null)));
    R.ok(`the row reads the two readings and the hours ("${(stopped ?? '').replace(/\s+/g, ' ').slice(0, 80)}")`, /6\.4 h · filed/.test(stopped ?? '') && (stopped ?? '').includes(fmtH(start1 + 6.4)));
    R.ok('the tile says all complete', (await PB.locator('[data-tile="rigs"]').getAttribute('data-tile-state')) === 'complete');
    ...
    R.ok('two rigs on the list, the second waiting for its stop hours', (await PB.locator('[data-rig-row]').count()) === 2 && /stop hours missing/.test(await row2.innerText()));
    await row2.click();
    ...
    // S20: the reading completes the paper — the office copy files, then back to the day
    await PB.waitForURL(/\/drill-checklist-file\//, { timeout: 20000 });
    await PB.locator('button:has-text("Done")').first().waitFor({ timeout: 30000 });
    await openDay(PB);
